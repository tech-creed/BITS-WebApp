    ...
    if (user_verify) {
        if (verify_db_find) {
            if (req.cookies.credLogin) {
                ...
                if (verify_cookie) {
                    res.render("password_change", { error: null })
                } else {
                    res.render("feedback_forgot", { error: 'Session Expired' })
                }
            } else {
                res.render("feedback_forgot", { error: 'Session Expires' });
            }
        } else {
            res.render("feedback_forgot", { error: 'Link Expires' });
        }
    } else {
        res.render("feedback_forgot", { error: 'Invalid User' });
    }
}
...
const LoginSignupGet = (req, res) => {
    res.render("login_signup", { error: null });
}
const ResetFeedbackGet = (req, res) => {
    res.render("feedback_forgot", { error: null });
}
...
const SignupPost = async(req, res) => {
    ...
    newUser.save().then((result) => {
        ...
        if (error.code == 11000) { res.render("login_signup", { error: 'Username Already Exist' }); }
    })
}
const LoginPost = async(req, res) => {
    ...
    if (!user) {
        res.render("login_signup", { error: "Username does Not Exist" });
    } else {
        if (await bcrypt.compare(userFind.password, user.password)) {
            ...
            res.render("login_signup", { error: "Password Incorrect" });
        }
        res.end()
    }
}
        ...
        if (user_reset.mail == req.body['mail_id']) {
            req_reset.save()
                .then(async(result) => {
                    ...
                    res.cookie('credLogin', tmp_token, { httpOnly: true });
                ...
                .catch((error) => {
                    console.log(error)
                    res.render("feedback_forgot", { error: 'Username / Mail Invaild' });
                })
        } else {
            res.render("feedback_forgot", { error: 'Username & Mail Mismatch' });
        }
    } else {
        res.render("feedback_forgot", { error: 'Username Invaild' });
    }
}
...
module.exports = { LoginSignupGet, LoginPost, SignupPost, AuthLogout, ResetFeedbackGet, Dashboard, PasswordResetPost, FeedbackPost, ChangePasswordPost, EmailVerificationGet }